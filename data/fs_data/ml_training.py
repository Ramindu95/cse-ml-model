...
class TrainingConfig:
    ...
    min_samples_per_field: int = 5 # Minimum samples required to train a model for a field (Adjusted for sample data)
    ...
    def __post_init__(self):
        if self.performance_threshold is None:
            self.performance_threshold = {
                'numeric': {'mae': 100000.0},
                'categorical': {'accuracy': 0.80}
            }
...
class TrainingDataManager:
    ...
    def load_training_data_from_db(self) -> Dict[str, List[Tuple[Union[str, List[Dict[str, Any]]], Any]]]:
        """
        Load training data from feedback database, including structured_content.
        Returns a dictionary where keys are field names and values are lists of
        (document_context, correct_value) tuples. document_context can be
        either a plain string or a list of structured text blocks.
        """
        training_data = {}
        
        try:
            with sqlite3.connect(self.db_path) as conn:
                # Get feedback data with document texts and structured content
                query = """
                ...
                feedback_data_rows = cursor.fetchall()
                
                for field_name, correct_value, extraction_data_json in feedback_data_rows:
                    try:
                        extraction_data = json.loads(extraction_data_json)
                        
                        # Prioritize structured_content if available, otherwise use full_text_context
                        document_context = extraction_data.get('structured_content')
                        if not document_context:
                            document_context = extraction_data.get('full_text_context')
                        
                        if not document_context:
                            logger.warning(f"Missing 'full_text_context' and 'structured_content' for field {field_name}. Skipping entry.")
                            continue
                        
                        if field_name not in training_data:
                            training_data[field_name] = []
                        
                        training_data[field_name].append((document_context, correct_value))
    ...
    def load_training_data_from_files(self) -> Dict[str, List[Tuple[Union[str, List[Dict[str, Any]]], Any]]]:
        """
        Load training data from manually curated JSON files in data_dir.
        This function would need to be adapted if your manual files also contain structured data.
        For now, it assumes plain text.
        """
        training_data = {}
        ...
        for file_path in data_dir.glob("*.json"):
            try:
                ...
                field_name = file_path.stem.replace('_data', '')
                if field_name not in training_data:
                    training_data[field_name] = []
    ...
    def create_sample_training_data(self) -> Dict[str, List[Tuple[Union[str, List[Dict[str, Any]]], Any]]]:
        """
        Creates sample training data for testing and initial model training.
        This data is hardcoded and includes examples of structured content.
        """
        sample_data = {
            'revenue': [
                ("Revenue from operations LKR 1,234,567 thousand", 1234567.0),
                ("Total revenue amounted to 987,654 LKR thousands", 987654.0),
                ([{'text': 'Revenue', 'bbox': [100, 200, 150, 210], 'font_size': 12, 'is_bold': True, 'page_num': 1, 'line_id': 10, 'block_id': 5},
                  {'text': '1,524,047', 'bbox': [300, 200, 350, 210], 'font_size': 14, 'is_bold': False, 'page_num': 1, 'line_id': 10, 'block_id': 5}], 1524047.0),
            ...
            'profit_for_period': [
                ...
                ([{'text': 'Profit', 'bbox': [100, 300, 140, 310], 'font_size': 12, 'is_bold': True, 'page_num': 1, 'line_id': 20, 'block_id': 10},
                  {'text': 'for the period', 'bbox': [150, 300, 250, 310], 'font_size': 10, 'is_bold': False, 'page_num': 1, 'line_id': 20, 'block_id': 10},
                  {'text': '275,941', 'bbox': [300, 300, 350, 310], 'font_size': 14, 'is_bold': False, 'page_num': 1, 'line_id': 20, 'block_id': 10}], 275941.0),
            ...
            'operating_profit': [
                ...
                ([{'text': 'Operating', 'bbox': [100, 400, 160, 410], 'font_size': 12, 'is_bold': True, 'page_num': 1, 'line_id': 30, 'block_id': 15},
                  {'text': 'Profit/(Loss)', 'bbox': [170, 400, 250, 410], 'font_size': 10, 'is_bold': False, 'page_num': 1, 'line_id': 30, 'block_id': 15},
                  {'text': '429,603', 'bbox': [300, 400, 350, 410], 'font_size': 14, 'is_bold': False, 'page_num': 1, 'line_id': 30, 'block_id': 15}], 429603.0),
            ...
            'basic_eps': [
                ...
                ([{'text': 'Basic', 'bbox': [100, 500, 140, 510], 'font_size': 12, 'is_bold': True, 'page_num': 1, 'line_id': 40, 'block_id': 20},
                  {'text': '/', 'bbox': [145, 500, 150, 510], 'font_size': 10, 'is_bold': False, 'page_num': 1, 'line_id': 40, 'block_id': 20},
                  {'text': 'Diluted Earning Per Share (EPS)', 'bbox': [155, 500, 350, 510], 'font_size': 10, 'is_bold': False, 'page_num': 1, 'line_id': 40, 'block_id': 20},
                  {'text': '0.83', 'bbox': [400, 500, 430, 510], 'font_size': 14, 'is_bold': False, 'page_num': 1, 'line_id': 40, 'block_id': 20}], 0.83),
...
class TrainingOrchestrator:
    ...
    def _check_retrain_condition(self, field_name: str, current_samples: int, force_train: bool) -> bool:
        """
        ...
        4. force_train flag is True (overrides other conditions).
        """
        if force_train:
            logger.info(f"Field '{field_name}': Forcing retraining due to 'force_train' flag.")
            return True
    ...
    def run_training_cycle(self, force_train: bool = False, use_sample_data: bool = False):
        ...
        if use_sample_data:
            ...
            # If using sample data, force training to ensure models are always initialized/retrained
            # with the sample set, especially useful for initial setup and testing.
            force_train_for_sample = True 
        else:
            ...
            force_train_for_sample = False # Don't force train if not using sample data, rely on other conditions
        ...
        for field_name, data in training_data_dict.items():
            # Pass the combined force_train flag to the check condition
            if self._check_retrain_condition(field_name, len(data), force_train or force_train_for_sample):
            ...
            if data:
                ...
                try:
                    metrics = self.ml_manager.models[field_name].train(
                        document_contexts=[item[0] for item in data],
