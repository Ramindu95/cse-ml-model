...
# Import the FinancialParser instead of SelfLearningFinancialExtractor
from fs_parser import FinancialParser, ExtractedFinancialData, FinancialMetrics, Shareholder, Director
import logging
...
class PDFFinancialProcessor:
    """
    Enhanced PDF financial statement processor using the FinancialParser
    """
    ...
    def __init__(self, ml_model_dir: Optional[str] = "ml_models", db_path: Optional[str] = "extraction_learning.db"):
        ...
        """
        # Initialize the FinancialParser
        self.parser = FinancialParser(ml_model_dir=ml_model_dir, db_path=db_path)
        self.companies = []
    ...
    def process_single_pdf(self, filename: str, pdf_path: str) -> bool:
        ...
        # Use the FinancialParser
        try:
            # Call parse_document method of the FinancialParser
            extracted_data = self.parser.parse_document(
            ...
            logger.info(f"✅ Processed: {filename} → {folder_name}/fs_data.json (Confidence: {extracted_data.extraction_confidence:.2f}, Method: {extracted_data.extraction_method})")
            return True
